...
public class mainCoding extends JPanel implements ActionListener{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					mainCoding window = new mainCoding();
	...
	private void initialize() {
			...
			public void actionPerformed(ActionEvent e) {
				if (z == 1) {	
					...
					if (potion == 0) {
						...
						lblHi.setText("What is the answer to " + mainCoding.mathQuestion("I") + "?");
						textField.show();
						btnStartANew.hide();
						btnLoadASave.hide();
						button.hide();
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e){
				FileWriter fw = null;
				try {
					fw = new FileWriter("Images\\Save.txt");
		...
		lblHi.setBounds((int)(screenw * 0.7), (int)(screenh*0.77), 600, 180);
		...
		btnLoadASave.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (z == 1) {
					Scanner sc = null;
					try {
						sc = new Scanner(new File("Images\\Save.txt"));
					...
					if (mp > 0) {
						...
						lblHi.setText("What is the answer to " + mainCoding.mathQuestion("M") + "?");
						textField.show();
						btnStartANew.hide();
						btnLoadASave.hide();
						button.hide();
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (z == 0) {
					...
					lblNewLabel_1.setText("Welcome to the Mental Math Game!");
					lblHi.hide();
					lblHi.setBounds((int)(screenw * 0.7), (int)(screenh*0.78), 600, 180);
					button.setBounds((int)(screenw * 0.53), (int)(screenh*0.87), 180, 39);
					lblNewLabel_1.setBounds((int)screenw/100, (int)(screenh*0.85), 1018, 75);
					...
					if (c < -20000) {
						...
						if (combo > 5 && combot == true) {
							d = 4.5;
						}
						else if (combo > 2 && combot == true) {
							d = 4;
						}
						else {
							d = 3.5;
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (z == 3) {
					...
					combo = 0;
					btnMuteSound.show();
					lblHi.setBounds((int)(screenw/100), (int) (screenh * 0.72), 600, 200);
					button.setBounds((int)(screenw * 0.53), (int)(screenh*0.87), 180, 39);
					lblHi.show();
					lblNewLabel_1.hide();
					if (y < text.length && end == false) {
					...
					if (y == 2 || y == 7 || y == 13 || y == 20 || y == 23 || y == 30 || y == 32 || y == 36) { 
						...
						filename = music[y];
						musicOb.playMusic(filename);
						lblNewLabel_1.setBounds((int)(screenw * 0.42), (int)(screenh*0.05), 300, 100);
					...
					lblHi.setText("What is the answer to " + mainCoding.mathQuestion("A") + "?");
					textField.show();
					btnStartANew.hide();
					btnLoadASave.hide();
					button.hide();
	...
	public static double[] HP = {2, 3, 0, 4, 2, 3, 5, 0, 6, 5, 4, 3, 7, 0, 7, 11, 15, 9, 5, 1, 0, 20, 15, 0, 5, 6, 11, 30, 15, 14, 0, 350, 0, 100, 150, 999, 0, 1};
	...
	boolean battle = false, mag = false, end = false, combot = false;
	...
	public void actionPerformed(ActionEvent e) {
		...
		label.setBounds((int)(screenw * 0.25) + velx + xval, (int)(screenh*0.05) + vely + yval, 1100, 600);
		velx = -velx;
		vely = -vely;
	}
}
